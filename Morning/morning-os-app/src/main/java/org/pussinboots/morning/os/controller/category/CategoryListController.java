...
public class CategoryListController extends BaseController{
	...
	public Map<String,Object> list(
		...
		if (category != null) {
			...
			map.put("sort", ProductSortEnum.typeOf(sort).getType());// 排序方式
			map.put("category", category);// 当前类目信息
			map.put("products", basePageDTO.getList());// 商品列表
			map.put("pageInfo", basePageDTO.getPageInfo()); // 分页信息
			map.put("lowerCategories", lowerCategories);// 子类目列表
			map.put("upperCategories", upperCategories);// 父类目列表
		}
		return map;
	}
